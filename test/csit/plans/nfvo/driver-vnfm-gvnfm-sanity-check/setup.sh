...
GVNFMDRIVER_IP=`get-instance-ip.sh gvnfmdriver`
sleep_msg="Waiting_for_gvnfmdriver"
curl_path='http://'${GVNFMDRIVER_IP}':8484/openoapi/gvnfmdriver/v1/swagger.json'
wait_curl_driver CURL_COMMAND=$curl_path WAIT_MESSAGE='"$sleep_msg"' REPEAT_NUMBER=25 GREP_STRING="swagger"

echo SCRIPTS
# Pass any variables required by Robot test suites in ROBOT_VARIABLES
ROBOT_VARIABLES="-v MSB_IP:${MSB_IP} -v GVNFMDRIVER_IP:${GVNFMDRIVER_IP} -v SCRIPTS:${SCRIPTS}"
