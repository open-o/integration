...
driverInstantiateNfvoNsFuncTest
    ...
    Create Session    web_session    http://${MSB_IP}    headers=${headers}
    Set Request Body    ${json_string}
    ${resp}=    Post Request    web_session    /openoapi/gso/v1/nfvodrivers/${nsInstanceId}/instantiate     ${json_string}
    ${response_code}=     Convert To String      ${resp.status_code}
    List Should Contain Value    ${return_ok_list}   ${response_code}
    ${response_json}    json.loads    ${resp.content}
...
driverQueryNfvoNsProgressForCreateFuncTest
    ${headers}    Create Dictionary    Content-Type=application/json    Accept=application/json
    Create Session    web_session    http://${MSB_IP}    headers=${headers}
    ${result}=    Get Request    web_session    /openoapi/gso/v1/nfvodrivers/jobs/${jobId}
    BuiltIn.log   ${result}
...
driverQueryNfvoNsProgressForDeleteFuncTest
    ${headers}    Create Dictionary    Content-Type=application/json    Accept=application/json
    Create Session    web_session    http://${MSB_IP}    headers=${headers}
    ${result}=    Get Request    web_session    /openoapi/gso/v1/nfvodrivers/jobs/${jobId}
    BuiltIn.log   ${result}
