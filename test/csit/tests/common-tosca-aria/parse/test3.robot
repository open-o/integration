...
*** Variables ***
${input_file_path}      ${SCRIPTS}/../plans/common-tosca-aria/sanity-check/yamlinput/parse_input_invalid.yaml

*** Test Cases ***
Parse Test #3 - parse and report error on invalid TOSCA template
    [Documentation]        Check if Common TOSCA ARIA is able to handle error, caused by invalid TOCSA blueprint validation attempt
...
Check Response
    ...
    Should Be Equal                     ${response_issues_item_message}    field "unrecognized_section" is not supported in "topology_template"
